...
<body>
  ...
  <div class="container">  
    ...
    <tr>
          <td> </td>
          <td><input type="text" ng-model="addData.prdoucer_name" class="form-control" placeholder="Enter the producer name" ng-required="true" /></td>
          <td><input type="float" ng-model="addData.producer_rating" class="form-control" placeholder="Enter the producer rating" ng-required="true" /></td>
      ...
      <script type="text/ng-template" id="display">
                    ...
                    <td>{{data.prdoucer_name}}</td>
                    <td>{{data.producer_rating}}</td>


                    <td>
                        <button type="button" class="btn btn-primary btn-sm" ng-click="showEdit(data)">Edit</button>
                        <button type="button" class="btn btn-danger btn-sm" ng-click="deleteData(data.producer_id)">Delete</button>
                    </td>
                </script>
                <script type="text/ng-template" id="edit">
                  <td>{{data.producer_id}}<input type="hidden" ng-model="formData.producer_id" class="form-control" /></td>
                    <td><input type="text" ng-model="formData.prdoucer_name" class="form-control"  /></td>
                    <td><input type="float" ng-model="formData.producer_rating" class="form-control" /></td>

                    <td>
                        <input type="hidden" ng-model="formData.data.producer_id" />
...
app.controller('liveController', function($scope, $http){
    ...
    $scope.getTemplate = function(data){
        if (data.producer_id === $scope.formData.producer_id)
        {
            return 'edit';
        }
        else
    ...
    $scope.fetchData = function(){
        $http.get('select_producer.php').success(function(data){
    ...
    $scope.deleteData = function(producer_id){
        if(confirm("Are you sure you want to remove it?"))
        {
            $http({
                method:"POST",
                url:"delete_producer.php",
